...
class QuickSaleControllermultifirm extends Controller
{
...
<?php }
    ...
    public function quicksaleInvoice(Request $request)
    {
        ...
        $existing_invoice = Final_invoice::where($matchCase)->get();
        if (count($existing_invoice)==0) {

            if(isset($request->old_invoice_id) && $request->old_invoice_id!=''){
                $invoice_id = $request->old_invoice_id;
                $invoice = Final_invoice::find($invoice_id);
            }else{
                $invoice = new Final_invoice();
            }
            ...
            $invoice->save();
            $invoiceid = $invoice->final_invoice_id;
            if ($invoiceid) {
                // $customer_point= CustomerPoint::where('customer_id',$customer_id)->get();
                // if($customer_point->isEmpty()){
                if(isset($request->saved_invoice_id) && count($request->saved_invoice_id)>0){
                    foreach ($request->saved_invoice_id as $skey => $svalue) {
                        $sinvoice= Invoice::where('invoice_id',$svalue)->first();
                        if($sinvoice){
                            $sinvoice->final_invoice_id = $invoiceid;
                            $sinvoice->save();
                        }
                    }
                }
                ...
                if ($invoice_type == 1) {
                    if ($sitpackageRowCount != 0) {
                        $url = 'http://chinnievinnie.in/cvsalon/mypackinvoice/'. $invoiceid;
                        $message = " Hey ".$customer->name." Thanks for taking your package with CV Salon,".$url;
                    ...
                    return redirect()->back()->with('success', 'Invoice Saved');
                } else {
                    if ($sitpackageRowCount != 0) {
            ...
            // echo "hy";
            $invoiceid = $existing_invoice[0]->final_invoice_id;
    ...
    public function myinvoice($invoiceid)
    {
        ...
        $invoicedata = DB::table('invoice AS I')
            ->select('C.*', 'I.*', 'I.remark AS iremark','I.created_at as invoice_time')->leftJoin('customers AS C', 'C.id', '=', 'I.user_id')
            ->where('I.invoice_id', $invoiceid)
            ->first();
    ...
    function front_screan_data($request){
        ...
        $firm_ids = $request->saved_firm_id;
        if(count($request->service)>0){
             ...
             $service_id = array();
             $firms_qr = [];
             foreach ($request->service as $fkey => $fvalue) {
                $services = DB::table('service_brands AS SB')
                ->select('SG.group_name AS service_name','SG.firm_id','SB.brand_name as sbname','SB.service_brand_id','F.firm_name','F.qr_code as qd', 'SB.service_duration')
                ->leftJoin('service_group AS SG','SG.id','=','SB.service_id')
                ->leftJoin('firms AS F','SG.firm_id','=','F.id')
                ->where('SB.service_brand_id',$fvalue['brand'])
                ->first();
                $firms[] = $services->firm_name;
                $firms_data[$services->firm_name] = $services->firm_id;
                $service_id[$services->firm_name][] = $services->service_brand_id;
                $firms_qr[$services->firm_name] = $services->qd;
             }


             $firm = array_unique($firms);
             if(count($firm)>0){
                 $row = [] ;
                 $i=0;
                 foreach ($firm as $akey => $avalue) {
                    $row[$i]['billingdate'] = $request->billingdate;
                    $row[$i]['is_estimate'] = $request->is_estimate;
                    $total_amount = 0;
                    $total_discount = 0;
                    $firm_id = isset($firms_data[$avalue])?$firms_data[$avalue]:0;
                    $row[$i]['firm_name'] = $avalue;
                    $row[$i]['firm_id'] = $firm_id;
                    $row[$i]['firm_qr'] = isset($firms_qr[$avalue])?url('images/firms_qrcode/'.$firms_qr[$avalue]):'';
                    foreach ($request->service as $skey => $svalue) {
                        ...
                        if(in_array($svalue['brand'],$service_id[$avalue])){
                            ...
                            // ->leftJoin("product_brands AS PB",function($join){
                            //     $join->on('PB.sub_category_id','=','SB.sub_cate_id')
                            //         ->on('PB.service_id','=','SB.service_id');
                            // })
                            ->Join('firms AS F','SG.firm_id','=','F.id')
                            ->where('SB.service_brand_id',$svalue['brand'])
                            //->where('PB.id',$svalue['services'])->tosql();
                            ->first();
                     ...
                     $row[$i]['subtotalamount'] = $total_amount;
        ...
        $data = json_encode(['data'=>$row,'save_firm_id'=>$firm_ids]);
    ...
    function save_invoice(Request $request){
        ...
        if(isset($request->service)){
                $save_firm_id = $request->save_firm_id;
                $rservices = [];
                $service_total = [];
