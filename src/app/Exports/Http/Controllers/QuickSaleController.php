...
<?php }
    ...
    public function quicksaleInvoice(Request $request)
    {
        ...
        $totalgstdiscount = ($request->totalgstdiscount!='')?$request->totalgstdiscount:0;
        ...
        $serviceRowCount = ($request->servicePrice)?count($request->servicePrice):0;
        ...
        $gstdiscount = $request->servicesgstdiscount;
        $servicePriceTotal = $request->servicePriceTotal;
        $servicePrice = $request->servicePrice;
        $serviceAdd = $request->serviceAdd;
        ...
        if (count($existing_invoice)==0) {
            ...
            if (!empty($invoiceid)) {
                ...
                $rservice = ($request->services)?array_values($request->services):array();//product_brandid//IN-filter - brand_id
                $rserviceStaff = ($request->serviceStaff)?array_values($request->serviceStaff):array();;
                $free_service = ($request->free_service)?array_values($request->free_service):array();
                $offer_id = ($request->offer_id)?array_values($request->offer_id):array();
                    ...
                    if($request->customer_offer_id!=0){
                       ...
                       for ($i = 0; $i < $serviceRowCount; $i++) {
                        ...
                        $invoiceservice = new InvoiceServiceModel([
                            ...
                            'service_id' => $rbrand[$i],
                            'brand_id' => $rservice[$i],
                            'staff_id' => $rserviceStaff[$i],
                            'quantity' => $serviceQuantity[$i],
                            'price' => $servicePrice[$i],
                            'service_add' => ($serviceAdd[$i])?$serviceAdd[$i]:0,
                            'discount' => $serviceDisc[$i],
                            'cgst' => $servicesCgst[$i],
                            'sgst' => $servicesSgst[$i],
                            'gstdiscount' => $gstdiscount[$i],
                            'total_price' => $serviceTotal[$i],
                            'other_service_id' => $request->other_service_invoice_id,
                            'free_service' => ($free_service[$i])?$free_service[$i]:0,
                            'offer_id' => ($offer_id[$i])?$offer_id[$i]:0,
                            'service_status' => 1,
                        ]);
            ...
            if ($invoice_type == 1) {
                ...
                // }else {
                //     $url = 'http://chinnievinnie.in/cvsalon/myinvoice/'. $invoiceid;
                //     $message = "Hey ".$customer->name." Thanks for taking your package with CV Salon. ".$url;
                // }
                // Helper::sendMobileSMS($customer->contact, $message);
