                    ...
                    <div class="col-md-12">
                        <div class="dash_boxcontainner white_boxlist">
                            <div class="upper_basic_heading">
                                <span class="white_dash_head_txt">Edit staff <a href="{{ url('admin/staff') }}" class="btn margin0 btn-sm btn-mdb-color single-table-btn float-right waves-effect waves-light" ><i class="mdi mdi-arrow-left mr-1"></i>Back</a> </span>
                            </div>
                            ...
                            @endif
                            <div class="col-md-12">
                            <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" action="{{ route('staff.update', $staff->id) }}" method = "post" enctype="multipart/form-data">
                                <input type = "hidden" name = "_token" value = "<?php echo csrf_token(); ?>">
                                <input type="hidden" name="_method" value="patch" />
                                <input type="hidden" name="userid" id="userid" value="{{$staff->id}}">
                                
                                <div class="row">
                                  <div class="col-md-6">
                                    @if(!empty($staff->image))
                                    <img class="img img-responsive" src="{{ url('images/staff') }}/{{$staff->image}}" width="80px">
                                    @endif
                                        ...
                                        @if(!empty($branches))
                                          @foreach($branches as $branch)
                                            @php
                                              $id_branch = $branch->id;
                                            @endphp
                                            <option value="{{$branch->id}}" {{$id_branch == $staff->branch_id ? "selected":"" }}>{{$branch->name}}</option>
                                          @endforeach
                                        @endif
                                      </select>
                                  ...
                                  <div class="col-md-6">
                                    <div class="form-group">
                                      <label class="control-label" for="name">Name <span class="required">*</span> </label>
                                        <input type="text"  id="name" name="name" value="{{ old('name',$staff->name) }}" placeholder="Enter name"  class="form-control" autocomplete="off">
                                  ...
                                  <div class="col-md-6">
                                    <div class="form-group">
                                      <label class="control-label" for="contact">Contact <span class="required">*</span> </label>
                                        <input type="text"  id="contact" name="contact" value="{{ old('contact',$staff->phone_no) }}" placeholder="Enter contact"  class="form-control" autocomplete="off">
                                  ...
                                  <div class="col-md-6">
                                    <div class="form-group">
                                      <label class="control-label" for="email">Email </label>
                                        <input type="text"  id="email" name="email" value="{{ old('email',$staff->email) }}" placeholder="Enter email"  class="form-control" autocomplete="off">
                                  ...
                                  <div class="col-md-6">
                                    <div class="form-group">
                                      <label class="control-label" for="email">Password </label>
                                        <input readonly type="password"  id="password" name="password" value="{{ old('password',$staff->password) }}" placeholder="Enter password"  class="form-control" autocomplete="off">
