        ...
        <div class="modal" tabindex="-1" role="dialog" id="add_member_plan_cust">
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              ...
              <form action="{{ route('assignMemberPlanToCustomer')}}" method="POST" class="formvisit mt-2">
                <div class="modal-body">
                  <input type = "hidden" name = "_token" value = "<?php echo csrf_token(); ?>">
...
<script type="text/javascript">
  ...
  function MemberplanAssign(member_plan_id, plan_btn_id){
    let membership_type='';
    let minimum_req_amt='';
    ...
    if ( member_plan_id!="" ){
          $.ajax({
              ...
              data:{"_token": "{{ csrf_token() }}",'member_plan_id':member_plan_id},
              type:'POST',
              success:function(res) {
                console.log(res);
      ...
      if(offer_code!=""){
        $.ajax({
              ...
              success:function(res) {
                ...
                var result=(JSON.parse(res));
                if((result.msg==true)&&(result.data[0].offer_type=='Service')){
                    result.data.forEach((element,index) => {
                        addServiceRow();
                        // $('#groupidfirm_'+index).val(element.firmid);
                        $('#brand_'+index).val(element.service_brand_id);
                ...
                else if((result.msg==true)&&(result.data.offer_type=='Discounted')){
                  discount_type=result.data.discount_type;
                  discount=result.data.discount;
                  console.log(discount_type);
                  if(discount_type=='Fixed'){
                      $('#discountByValue').val(discount);
                    }
                    else if(discount_type=='Percent'){
                      $('#discountByPercent').val(discount);
                    }
                }
                else if(result.msg=='Not allowed'){
            ...
            if(res.msg==true){
              res.data.forEach((element, index) => {
                new_row=`<tr>
                            ...
                            <td class="td-sm">${element.email} <br> ${element.contact}</td>
                            <td class="td-sm">
