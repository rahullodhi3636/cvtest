      ...
      <div class="col-sm-12">
        ...
        <div class="col-sm-9">
          <ul class="nav nav-tabs md-tabs profile-nav">
            ...
            <li class="nav-item">
              <a class="nav-link" id="invoice-tab-md" data-toggle="tab" href="#invoice-md" role="tab" aria-controls="transaction-md"
     ...
     <div class="row">
       ...
       <div class="col-lg-4 col-md-12 col-12">
         <div class="single-price serverborder">
          ...
          <form id="serviceform_{{$servicelist->id}}" method="post" class="form-inline my-2 my-lg-0" >
              <input type = "hidden" name = "_token" value = "<?php echo csrf_token(); ?>">
              <input name="id" type="hidden" value="{{$servicelist->id}}">
              <input name="name" type="hidden" value="{{$servicelist->name}}">
              <input name="price" type="hidden" value="{{$servicelist->service_price}}">
              <input name="customer_id" type="hidden" value="{{$customer->id}}">
              <button class="btn btn-success btn-block" onclick="getservice('{{$servicelist->id}}')" type="button">Add to cart</button>
...
<div class="tab-pane fade" id="invoice-md" role="tabpanel" aria-labelledby="invoice-tab-md">
  <div class="userleftbody">
   <div class="row">
     <div class="col-lg-12 col-md-12 col-12">
      ...
      <div class="table-responsive">
        ...
        @php
        $invoicetransaction = DB::table('invoice_transactions')->where('invoice_transactions.customer_id',$customer->id)->get();
        if(!empty($invoicetransaction)){
        foreach($invoicetransaction as $invoice){
        @endphp
        <tr>
          <td>{{$invoice->payment_mode}}</td>
          <td>{{$invoice->transaction_amount}} Rs</td>
          <td>{{$invoice->reward_point}} point</td>
          <td>{{$invoice->total_pay_amount}} Rs</td>
          <td>{{$invoice->remaining_amount}} Rs</td>
          <td>{{date('d-m-Y',strtotime($invoice->transaction_date))}}</td>
          <td>
            <a href="javascript:void(0)" onclick="viewTransaction('{{$invoice->invoice_transaction_id}}')" type="button" class="btn btn-danger waves-effect waves-light btn-sm">view</a>
            <!-- <a href="{{url('admin/invoice')}}/{{$invoice->invoice_transaction_id}}" target="_blank" type="button" class="btn btn-success waves-effect waves-light btn-sm">invoice</a> -->
            <?php /*if ($invoice->remaining_amount != 0):*/ 
              /*$remainTransaction = DB::table('remaining_transaction_details')->where('invoice_transaction_id',$invoice->invoice_transaction_id)->first();
              if (!empty($remainTransaction)):*/
            ?>
              <!-- <a href="javascript:void(0)" onclick="viewTransaction()" type="button" class="btn btn-danger waves-effect waves-light btn-sm">view</a> -->
            <?php /*endif*/ ?>
          </td>
        </tr>
        @php
...
<!-- getservice Modal -->
  <div class="modal" id="getservicemodal">
    <div class="modal-dialog modal-sm">
      <div class="modal-content" style="background: #000;">
        ...
        <!-- Modal body -->
        <div class="">
         <form id="cartform" method="post" class="editform" >
          <input type = "hidden" name = "_token" value = "<?php echo csrf_token(); ?>">
          <input name="customer_id" type="hidden" value="{{$customer->id}}">
          <div class="form-group">
            <label>Select Brand</label>
            <select class="form-control" id="brand" name="brand" onchange="showproductDiv(this.value)">
              <option value="">Select</option>
            </select>
          </div>
          <div id="productsdiv" style="display: none;">
            ...
            <button class="btn btn-success btn-block" onclick="addtocart()" type="button">Add to cart</button>
          </div>
  ...
  <div class="modal" id="paidmodal">
    <div class="modal-dialog  modal-lg">
      <div class="modal-content" style="background: #000;">
        ...
        <!-- Modal body -->
        <div class="" id="paidmodalformbody">
          <form id="paidmodalform" method="post">
            <div class="cart-modalbody">
              <div class="cart-items-container" id="cart-item-body">
                ...
                <div class="form-group">
                  ...
                  <input readonly type="text" class="form-control" name="totalamount" id="totalamount" value="">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input readonly type="number" class="form-control" name="payamount" id="payamount" placeholder="payable amount" value="">
                </div>
                <div class="form-group">
                  <button id="checkoutbtn" type="button" onclick="paynow()" class="btn btn-info btn-block waves-effect waves-light">Pay</button>
                  <a href="javascript:void(0)" class="btn btn-warning btn-block waves-effect waves-light">Cancel</a>
                </div>
              </div>
            </div>
          </form>
...
<script type="text/javascript">
  function paynow() {
    var formdata = $("#paidmodalform").serialize();
    $.ajax({
      url:'{{url("admin/makePayment")}}',
      type:'POST',
      data:formdata,
      success:function(res) {
        if (res == "Done") {
  ...
  function payRemainAmount(historyid) {
    if (historyid != "") {
      $.ajax({
        url:'{{url("admin/makePaymentForm")}}/'+historyid,
        type:'GET',
        dataType:'JSON',
        success:function(res) {
          $("#paidmodal").modal('show');
          $("#totalamount").val(res.pay_amount);
          $("#payamount").val(res.pay_amount);
          $("#transactionid").val(historyid);
        }
      });
    }
  }
  function viewTransaction(invoiceid) {
    if (invoiceid != "") {
      $.ajax({
        url:'{{url("admin/viewhistory")}}/'+invoiceid,
        type:'GET',
        success:function(res) {
          if (res != "") {
            $("#viewhistorymodal").modal('show');
            $("#historybody").html(res);
          }
        }
      });
    }
  }
  function showproductDiv(bid) {
    if (bid != "") {
      $.ajax({
        url:'{{url("customer/getproducts")}}/'+bid,
        type:'GET',
        success:function(res) {
          if (res != "") {
            $("#productsdiv").show();
  ...
  function getservice(serviceid) {
    var formdata = $("#serviceform_"+serviceid).serialize();
    $.ajax({
      url:'{{url("getbrand")}}',
      type:'POST',
      data:formdata,
